...
class TestImageSimilarity(unittest.TestCase):
    ...
    @parameterized.expand([MatchType.GRAYSCALE,
                           MatchType.COLOUR])
    def test_image_similarity_should_return_all_matches(self, match_type):
        similarity = assertNotRaises(ImageSimilarity(
            self.button_image, self.base_image).find_all_matches(match_type=match_type).save_image,
            self._testMethodName + str(match_type))
